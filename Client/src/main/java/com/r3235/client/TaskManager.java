...
public class TaskManager {
    ...
    public Task getTask(String stringTask, boolean isScriptCommand) {
        ...
        try {
            switch (command) {
                case "login":
                    login = stringTask.trim().split(" ", 2)[1].trim().split(" ", 2)[0].trim();
                    password = stringTask.trim().split(" ", 2)[1].trim().split(" ", 2)[1].trim();
                    task = new Task(CommandType.LOGIN);
                    break;
                case "reg":
                    login = stringTask.trim().split(" ", 2)[1].trim().split(" ", 2)[0].trim();
                    password = stringTask.trim().split(" ", 2)[1].trim().split(" ", 2)[1].trim();
                    task = new Task(CommandType.REG);
                    break;
                case "help":
                ...
                case "add":
                    if (!isScriptCommand) {
                        ...
                        Product productA = ReadCSV.toCSV(stringTask.trim().split(" ", 2)[1].trim());
                ...
                case "add_if_min":
                    if (!isScriptCommand) {
                        ...
                        Product product = ReadCSV.toCSV(stringTask.trim().split(" ", 2)[1].trim());
       ...
       if (task!=null) task.setLoginPassword(login,password);
        return task;
    }
}
